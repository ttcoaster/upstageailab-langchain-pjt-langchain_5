...
def main():
    ...
    if answer1["success"]:
        print("답변:", answer1["response"])
        print("출처:", answer1["sources"])
    else:
        print("오류:", answer1["error"])
    
    print("\n" + "="*50 + "\n")
    
    # 3. 두 번째 질문 (메모리 기능 확인)
    print("=== 두 번째 질문 (메모리 기능 확인) ===")
    answer2 = processor.query("방금 전에 물어본 질문이 뭐였나요?", return_sources=True)
    if answer2["success"]:
        print("답변:", answer2["response"])
        print("출처:", answer2["sources"])
    else:
        print("오류:", answer2["error"])
